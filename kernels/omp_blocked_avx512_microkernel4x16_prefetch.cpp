...
void omp_blocked_avx512_microkernel4x16_prefetch(int M, int N, int K, float* A, float* B, float* C) {
    const int m_full = (M / 4) * 4;
    std::vector<float> Bp;

    // Keep one OpenMP team alive across the whole GEMM. For each (jj, kk) panel,
    // one thread packs B and the team then consumes that shared packed panel.
    #pragma omp parallel
    {
